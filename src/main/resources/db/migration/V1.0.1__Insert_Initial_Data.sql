INSERT INTO "user" (email,
                    ...
                    phone_number,
                    birth_date)
VALUES ('admin@rideshare.com',
        '$2a$10$rG4MF4MqZrUrU4msCXmJoeCY1Zd8CXy0IRmEBQO/8oHzYrb.KRXJ2', -- password: 'password'
        'System',
        'Admin',
        '+40700000000',
        '1990-01-01');
...
-- Get the admin user_id for references (using currval since it was just inserted)
DO
$$
    DECLARE
        admin_id bigint;
    BEGIN
        admin_id := currval(pg_get_serial_sequence('"user"', 'user_id'));
        ...
        -- Insert permissions with action:resource format
        INSERT INTO permission (name, created_by)
        VALUES ('manage:user', admin_id),
               ('manage:role', admin_id),
               ('create:location', admin_id),
               ('read:location', admin_id),
               ('update:location', admin_id),
               ('delete:location', admin_id),
               ('create:ride', admin_id),
               ('read:ride', admin_id),
               ('update:ride', admin_id),
               ('delete:ride', admin_id),
               ('create:booking', admin_id),
               ('cancel:booking', admin_id),
               ('read:booking', admin_id),
               ('delete:booking', admin_id),
               ('create:vehicle', admin_id),
               ('update:vehicle', admin_id),
               ('delete:vehicle', admin_id),
               ('read:vehicle', admin_id),
               ('create:rating', admin_id);

        -- Assign admin role to admin user
        INSERT INTO user_roles (user_id, role_id, granted_by)
        VALUES (admin_id, (SELECT role_id FROM role WHERE name = 'ADMIN'), admin_id);

        -- Insert role-permission mappings
        WITH roles AS (SELECT role_id, name
                       FROM role),
             permissions AS (SELECT permission_id, name
                             FROM permission)
        INSERT
        INTO role_permissions (role_id, permission_id, assigned_by)
        SELECT r.role_id,
               p.permission_id,
               admin_id
        FROM roles r
        ...
        WHERE (r.name = 'ADMIN') -- Admin gets all permissions
           OR (r.name = 'MODERATOR' AND p.name IN (
                                                'create:ride', 'update:ride', 'delete:ride', 'read:ride',
                                                'read:booking', 'update:booking',
                                                'create:vehicle', 'update:vehicle', 'delete:vehicle', 'read:vehicle'
            ))
           OR (r.name = 'USER' AND p.name IN (
                                              'read:location',
                                              'create:ride', 'read:ride',
...
INSERT INTO location (city, county)
